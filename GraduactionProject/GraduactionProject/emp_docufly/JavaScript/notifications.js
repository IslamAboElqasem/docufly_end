function filterTable() {
    ...
    if (!searchCode && searchType === "" && searchStatus === "" && !dateRangeInput) {
    ...
    tableRows.forEach((row) => {
        ...
        const statusMatch =
            searchStatus === "" || requestStatus.includes(searchStatus);
...
document.getElementById("dateRangeIcon").onclick = () =>
...
function updatePagination() {
    const totalRows =
        document.getElementById("data-table").getElementsByTagName("tr").length - 1;
...
function sortRowsByDate() {
    ...
    rows.sort((rowA, rowB) => {
        const dateA = new Date(rowA.cells[2].innerText); // التاريخ في الصف A
        const dateB = new Date(rowB.cells[2].innerText); // التاريخ في الصف B
...
function showPopup() {
    // إخفاء نافذة الصورة
    var imageModal = bootstrap.Modal.getInstance(
...
function submitRejection() {
    ...
    window.location.href =
        "http://127.0.0.1:5502/HTML/employee%20req%20html/5tab%20moafka%20mn%20elrsayel%20elmab3ota%20mn%20el%20itc.html"; // استبدل هذا بالرابط الذي تريده
}
